...
function HRDashboard() {
  ...
  }, [loadRequests, loadDepartmentsStatus, loadDepartments, loadTodayStatus, checkForNewNotifications, checkTomorrowHoliday]);
  ...
  const loadRequests = useCallback(async () => {
    ...
    const ownRequests = allReqs.filter(r =>
      ...
      String(r.employee_id) === String(user.id) ||
      String(r.employeeId) === String(user.id) ||
      r.role === 'hr'
    );
  ...
  const loadDepartmentsStatus = useCallback(async () => {
  ...
  const handleAction = useCallback(async (requestId, action, reason) => {
    if (action === 'approve') {
  ...
  }, [loadRequests, loadDepartmentsStatus]);

  const checkTomorrowHoliday = useCallback(async () => {
